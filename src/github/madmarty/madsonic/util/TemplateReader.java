...
public class TemplateReader {
	...
	public static <serverEntries> SrvSettings getTemplates(Context context){
    ...
    try {
           ...
           File f = new File(FileUtil.getMadsonicDirectory(context), "madsonic.xml");
           if(f.exists()) { 
        	   doc = docBuilder.parse (f);
           } else {
                f = new File(FileUtil.getDefaultDownloadDirectory(), "madsonic.xml");
                if(f.exists()) {
             	   doc = docBuilder.parse (f);
                }
           }

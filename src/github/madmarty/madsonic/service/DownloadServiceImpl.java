...
import github.madmarty.madsonic.util.CurrentPlayBuffer;
import github.madmarty.madsonic.util.LRUCache;
import github.madmarty.madsonic.util.Logger;
import github.madmarty.madsonic.util.PandoraPlayBuffer;
...
public class DownloadServiceImpl extends Service implements DownloadService {
    ...
    private final PandoraPlayBuffer pandoraPlayBuffer = new PandoraPlayBuffer(this);
    private final CurrentPlayBuffer currentPlayBuffer = new CurrentPlayBuffer(this);
	...
	@Override
	public synchronized void setPandoraPlayEnabled(boolean enabled) {
        if (pandoraPlay == enabled) {
            return;
        }
	...
	@Override
	public boolean isPandoraPlayEnabled() {
        return pandoraPlay;
	}
    ...
    @Override
    public CurrentPlayBuffer getCurrentPlayBuffer() {
        return currentPlayBuffer;
    }
  ...
  //          	if (secondaryProgress == -1 || secondaryProgress >= position)
	...
	public synchronized void setPlayerState(final PlayerState playerState) {
        ...
        Util.broadcastPlaybackStatusChange(this, null, playerState);
    ...
    protected synchronized void checkDownloads() {
        if (!Util.isExternalStoragePresent() || !lifecycleSupport.isExternalStorageAvailable() || Util.isOffline(getBaseContext()) ) {
            return;
        }
    ...
    private synchronized void checkPandoraPlay() throws Throwable {
         ...
         if (size < listSize) {
        	 ...
        	 List<MusicDirectory.Entry> currentPlaylist = currentPlayBuffer.getall();
        	 if (currentPlaylist.size() != 0) {
        		 id = Integer.valueOf(currentPlaylist.get(currentPlaylist.size() - 1).getId());
        	 }
        	 
        	 if (id > 0) {
                 if (pandoraPlayBuffer.size() < 2) {
                	 pandoraPlayBuffer.refill(id);
                     Util.toast(getBaseContext(), "refilled");

                 }
        	 }
        	 
             Util.toast(getBaseContext(), "PANDORA-BUFFER: " + pandoraPlayBuffer.size());
        	 
             for (MusicDirectory.Entry song : pandoraPlayBuffer.get(listSize - size)) {
         ...
         // Only shift playlist if playing song #6 or later.
         if (currIndex > 7) {
             int songsToShift = currIndex - 2;
             for (MusicDirectory.Entry song : pandoraPlayBuffer.get(songsToShift)) {

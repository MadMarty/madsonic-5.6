...
import github.madmarty.madsonic.service.parser.NowplayingSongsParser;
...
//    }
    ...
    public MusicDirectory getPlaylist(String id, String name, Context context, ProgressListener progressListener) throws Exception {
        ...
        try {
			...
			File playlistFile = FileUtil.getPlaylistFile(context, name);
    ...
    public MusicDirectory getAlbumList(String type, int size, int offset, Context context, ProgressListener progressListener) throws Exception {
        ...
        Reader reader = getReader(context, progressListener, "getAlbumList", null, Arrays.asList("type", "size", "offset"), Arrays.<Object>asList(type, size, offset));
        try {
    ...
    @Override
    public MusicDirectory getPandoraSongs(int id, Context context, ProgressListener progressListener) throws Exception {
        HttpParams params = new BasicHttpParams();
        HttpConnectionParams.setSoTimeout(params, SOCKET_READ_TIMEOUT_GET_RANDOM_SONGS);

        Reader reader = getReader(context, progressListener, "getPandoraSongs", params, "id", id);
        try {
            return new RandomSongsParser(context).parse(reader, progressListener);
        } finally {
            Util.close(reader);
    ...
    @Override
    public String getVideoUrl(String format, int maxBitrate, Context context, String id, boolean useFlash) throws Exception {
	...
	public String getHlsUrl(String id, int bitRate, Context context) throws Exception {
        ...
        builder.append("&id=").append(id);
		if(bitRate > 0) {
			builder.append("&maxBitRate=").append(bitRate);
		}

        String url = rewriteUrlWithRedirect(context, builder.toString());
        LOG.info( "Using hls URL: " + url);
        return url;
	}
	...
	@Override
	public MusicDirectory getSongsByGenre(String genre, int size, int offset, Context context, ProgressListener progressListener) throws Exception {
    	checkServerVersion(context, "1.9", "Genres not supported.");
        HttpParams params = new BasicHttpParams();
        HttpConnectionParams.setSoTimeout(params, SOCKET_READ_TIMEOUT_GET_RANDOM_SONGS);
        ...
        Reader reader = getReader(context, progressListener, "getRandomSongs", params, parameterNames, parameterValues);
	...
	public MusicDirectory getTopTrackSongs(String artist, int size, Context context, ProgressListener progressListener) throws Exception {
        ...
        checkServerVersion(context, "1.11.5", "TopTrack Songs not supported.");
		HttpParams params = new BasicHttpParams();
        HttpConnectionParams.setSoTimeout(params, SOCKET_READ_TIMEOUT_GET_RANDOM_SONGS);
        ...
        Reader reader = getReader(context, progressListener, "getTopTrackSongs", params, parameterNames, parameterValues);
        try {
            return new RandomSongsParser(context).parse(reader, progressListener);
        } finally {
            Util.close(reader);
	...
	@Override
	public MusicDirectory getNowplayingSongs(Context context, ProgressListener progressListener) throws Exception {
        checkServerVersion(context, "1.7", "Nowplaying Songs not supported.");
        ...
        Reader reader = getReader(context, progressListener, "getNowPlaying", params, parameterNames, parameterValues);
        try {
            return new NowplayingSongsParser(context).parse(reader, progressListener);
        } finally {
            Util.close(reader);
        }
    }
	...
	@Override
	public Bitmap getAvatar(Context context, String username, int size, int savesize, ProgressListener progressListener) throws Exception
	{
		// Return silently if server is too old
		checkServerVersion(context, "1.8", "Avatar Picture is not supported.");
...
//						new ErrorParser(context).parse(new InputStreamReader(in, Constants.UTF_8));
					...
					bitmap = FileUtil.getSampledBitmap(bytes, size);
				}
				finally
				{
					Util.close(in);

...
//    }
	...
	public String getVideoUrl(String format, int maxBitrate, Context context, String id, boolean useFlash) throws Exception {
        return musicService.getVideoUrl(format, maxBitrate, context, id, useFlash);
	}
...
//    }
	...
	public List<PodcastChannel> getPodcastChannels(boolean refresh, Context context, ProgressListener progressListener) throws Exception {
		...
		if (result == null) {
			if(!refresh) {
				//FIXME:TODO
				result = FileUtil.deserialize(context, getCacheName(context, "podcast"));
			}
	...
	public MusicDirectory getPandoraSongs(int id, Context context, ProgressListener progressListener) throws Exception {
		return musicService.getPandoraSongs(id, context, progressListener);
	}
	...
	public Bitmap getAvatar(Context context, String username, int size, int savesize, ProgressListener progressListener) throws Exception {
		return musicService.getAvatar(context, username, size, savesize, progressListener);
	}

	@Override
	public MusicDirectory getNowplayingSongs(Context context, ProgressListener progressListener) throws Exception {
		return musicService.getNowplayingSongs(context, progressListener);
	}

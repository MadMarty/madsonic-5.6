...
public class SelectAlbumActivity extends SubsonicTabActivity {
    ...
    private boolean isTopTracklist;
    private boolean isGenreTracklist;
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        int getTopTracks = getIntent().getIntExtra(Constants.INTENT_EXTRA_NAME_TOPTRACK, 0);
   ...
   //   TextView songCountView = (TextView) findViewById(R.id.artist_song_count);
        ...
        playNowButton.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
            	 SharedPreferences myPreference=PreferenceManager.getDefaultSharedPreferences(SelectAlbumActivity.this);
...
//        });
        ...
        isTopTracklist = getTopTracks > 0;
        isGenreTracklist = genreName != null;
    ...
    private void getTopTrackSongs(final String artist, final int size) {
    	
    //	setTitle("TopTrcccccacks");
    	
        TextView text = (TextView) findViewById(R.id.select_album_artist);
        text.setText("TopTracks");
        text.setVisibility(View.VISIBLE);
        
        new LoadTask() {
            @Override
            protected MusicDirectory load(MusicService service) throws Exception {
                return service.getTopTrackSongs(artist, size, SelectAlbumActivity.this, this);
            }
            
            @Override
            protected void done(MusicDirectory result) {
              Intent intent = getIntent();
              intent.putExtra(Constants.INTENT_EXTRA_NAME_TOPTRACK, result.getChildren().size());
              super.done(result);
            	
              int songCount = result.getChildren().size();
              String s = getResources().getQuantityString(R.plurals.select_album_n_songs, songCount, songCount);
              TextView songCountView = (TextView) findViewById(R.id.select_album_song_count);
              songCountView.setText(s);
...
//    intent.setDataAndType(Uri.parse(MusicServiceFactory.getMusicService(this).getVideoStreamUrl(maxBitrate, this, entry.getId())), "video/*");
    ...
    private abstract class LoadTask extends TabActivityBackgroundTask<MusicDirectory> {
        ...
        protected void done(MusicDirectory result) {
        	...
        	MusicDirectory.Entry ParentEntry = new MusicDirectory.Entry();
        	
        	ParentEntry.setArtist(result.getName());
        	ParentEntry.setId(result.getParentId());
        	ParentEntry.setCoverArt(result.getId());
            ...
            if (songCount > 0 || ParentEntry.getCoverArt() != null ) {
            	if (getIntent().hasExtra(Constants.INTENT_EXTRA_NAME_TOPTRACK)) {
            		//
            	} else {
            		if (ParentEntry != null) {
            			entryList.addHeaderView(createHeader2(result, ParentEntry));
            		}
            	}
...
//				unpinButton.setVisibility(View.VISIBLE);
            ...
            entryList.setAdapter(entryAdapter = new EntryAdapter(SelectAlbumActivity.this, getImageLoader(), entries,true, true));
    ...
    private View createHeader(final MusicDirectory directory, Entry parentEntry) {
//       private View createHeader(final MusicDirectory directory ) {
    	
        List<MusicDirectory.Entry> entries = directory.getChildren();
        View header = LayoutInflater.from(this).inflate(R.layout.select_albumartist_header, entryList, false);
//        View header = LayoutInflater.from(this).inflate(R.layout.select_album_header, entryList, false);

        View coverArtView = header.findViewById(R.id.select_album_art);
        
//      getImageLoader().loadImage(coverArtView, parentEntry, false, true);
//      getImageLoader().loadImage(coverArtView, entries.get(0), false, true);
          
        boolean offline = Util.isOffline(this);

        final ImageView starView = (ImageView) header.findViewById(R.id.select_albumartist_star);
        ...
        if (songCount < 1) {
        	...
        	songCountView.setVisibility(View.VISIBLE);
        	s = getResources().getQuantityString(R.plurals.select_album_n_videos, dirCount, dirCount);
        }
        songCountView.setText(s.toUpperCase());
    ...
    private View createHeader2(final MusicDirectory directory, Entry parentEntry) {
      ...
      View coverArtView = header.findViewById(R.id.select_album_art);
       ...
       if (isBlockedlist || isGenreTracklist) {
    	   getImageLoader().loadImage(coverArtView);
       } else {
	      getImageLoader().loadImage(coverArtView, parentEntry, false, true);
       }
      ...
      starView.setImageResource(directory.isStarred() ? android.R.drawable.btn_star_big_on : android.R.drawable.btn_star_big_off);
      starView.setVisibility(offline || isPlaylist || isGenreTracklist || isBlockedlist ? View.GONE : View.VISIBLE);
      ...
      shareView.setVisibility(offline || isPlaylist || isBlockedlist || isTopTracklist ? View.GONE : View.VISIBLE);

      
      final ImageView toptrackView = (ImageView) header.findViewById(R.id.select_albumartist_toptrack);
      toptrackView.setOnClickListener(new View.OnClickListener() {
          @Override
          public void onClick(View view) {
        	  
//        	  String x1 = directory.getChildren().get(0).getArtist();
//        	  String x2 = directory.getName();
        	  
        	  getTopTrackSongs(directory.getChildren().get(0).getArtist(), 9);
//        	  getTopTrackSongs(directory.getName(), 9);
          }
      });      
      
      toptrackView.setVisibility(offline || isPlaylist || isBlockedlist  || isTopTracklist ? View.GONE : View.VISIBLE);
...
//    shareMadsonicView.setVisibility(offline || isPlaylist || isBlockedlist ? View.GONE : View.VISIBLE);
      ...
      if (artists.size() == 1 ) {
    	  ...
    	  songCountView.setText(" ");
//          artistView.setVisibility(View.GONE);
      }

      if (isGenreTracklist || isBlockedlist ) {
          artistView.setVisibility(View.GONE);
      }
      ...
      if (isVideolist) {
    	  ...
    	  if (songCount < 1) {
    		  ...
    		  songCountView.setVisibility(View.VISIBLE);
    		  shareView.setVisibility(offline || isPlaylist || isBlockedlist  || isGenreTracklist || isTopTracklist ? View.GONE : View.VISIBLE);
    		  s = getResources().getQuantityString(R.plurals.select_album_n_songs, songCount, songCount);
    	  }
    	  ...
    	  if (dirCount > 0) {
    		  songCountView.setVisibility(View.VISIBLE);
    		  shareView.setVisibility(View.GONE);
    		  s = getResources().getQuantityString(R.plurals.select_album_n_albums, entries.size(), entries.size());
    	  }
      }       
	  ...
	  if (songCount > 0) {
		 ...
		 if ( s == null) {
			 artistView.setText("");
		      ...
		      artistView.setText(s.toUpperCase());
		 }
	  }

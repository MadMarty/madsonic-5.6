...
public class SettingsActivity extends PreferenceActivity implements SharedPreferences.OnSharedPreferenceChangeListener {
	...
	private ListPreference randomSize;
    ...
    private CheckBoxPreference jump2PlayerEnabled;
    private CheckBoxPreference jump2PlaylistEnabled;
    private CheckBoxPreference fullscreenEnabled;
...
//      coverSize = (ListPreference) findPreference(Constants.PREFERENCES_KEY_COVER_SIZE);
		...
		randomSize = (ListPreference) findPreference(Constants.PREFERENCES_KEY_RANDOM_SIZE);
        ...
        jump2PlayerEnabled = (CheckBoxPreference) findPreference(Constants.PREFERENCES_KEY_JUMP2PLAYER);
        jump2PlaylistEnabled = (CheckBoxPreference) findPreference(Constants.PREFERENCES_KEY_JUMP2PLAYLIST);
        fullscreenEnabled = (CheckBoxPreference) findPreference(Constants.PREFERENCES_KEY_THEME_FULLSCREEN);
...
//        });
        ...
        ///////////////////////////// IMPORT XML TEMPLATE ////////////////////////
        File template = new File(FileUtil.getMadsonicDirectory(this), "madsonic.xml");
        if(!template.exists()) {
       	    template = new File(FileUtil.getDefaultDownloadDirectory(), "madsonic.xml");
        }
        if (template != null) {
        ...
        findPreference(Constants.PREFERENCES_KEY_IMPORT_TEMPLATE).setOnPreferenceClickListener(new Preference.OnPreferenceClickListener() {
            ...
            public boolean onPreferenceClick(Preference preference) {
				...
				try { importTemplate = TemplateReader.getTemplates(getBaseContext()).getChildren();}
				catch (Exception e) {
		...
		if (TemplateReader.getTemplates(getBaseContext()) == null) {
    ...
    public void applyTheme() {
        ...
        boolean fullscreen = Util.getThemeFullscreen(this);
        setTheme ( Util.getThemeStyle(this, theme, fullscreen) );     
    } 
...
//        defaultVideoplayer.setSummary(defaultVideoplayer.getEntry());
		...
		randomSize.setSummary(randomSize.getEntry());
    ...
    private void setHideMedia(boolean hide) {
        File nomediaDir = new File(FileUtil.getMadsonicDirectory(this), ".nomedia");
